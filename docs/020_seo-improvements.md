...
| 投稿タイプ | 頻度 | 内容 | 狙い |
|-----------|------|------|------|
| 日替わりセール告知 | 毎日 0:05 JST | 当日のセール上位3作品 + `/sale` リンク | /sale への流入・拡散 |
| 週間ランキング | 毎週月曜 9:00 | TOP5 タイトル + `/ranking` リンク | /ranking への流入 |
| 女優新着通知 | 不定期（新作検知時） | 女優名 + 新作タイトル + 女優ページリンク | /actress/[id] への流入 |

**実装:** `workers/x-post.ts` を作成し `wrangler.toml` に cron トリガーを追加。
投稿文は `SEOとサイト育成・拡散戦略.md` に記載の WOW/知っトク/WANT フレームワークを使う。
