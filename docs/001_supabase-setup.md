...
- [x] `notification_subscriptions` テーブル（id, user_id（nullable）, endpoint（unique）, keys, notification_type, created_at）。`user_id` は nullable でゲストのセール速報購読を許可（`notification_type='sale'`）
- [x] `notification_queue` テーブル（id, user_id（nullable）, endpoint（nullable・ゲスト sale 送信先）, type, payload, status, sent_at, created_at）
